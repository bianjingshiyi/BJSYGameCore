...
namespace BJSYGameCore.UI
{
    ...
    /// </summary>
    /// <remarks>
    /// 查看Controller（Animator的Layer）
    /// </remarks>
    [CustomEditor(typeof(UIController))]
    class UIControllerEditor : Editor
    {
        ...
        {
            base.OnInspectorGUI();
            Animator animator = (target as UIController).GetComponent<Animator>();
            if (animator.runtimeAnimatorController is AnimatorController controller)
            {
                ...
                foreach (AnimatorControllerLayer layer in controller.layers)
                {
                    ...
                    if (m.Success)
                    {
                        ...
                        {
                            //Controller当前状态
                            EditorGUILayout.LabelField((target as UIController)[controllerName], GUILayout.Width(100));
                        }
                        ...
                        if (controllerFoldDic[controllerName])
                        {
                            ...
                            foreach (ChildAnimatorState state in layer.stateMachine.states)
                            {
                                ...
                                {
                                    (target as UIController)[controllerName] = state.state.name;
                                }
                                if (GUILayout.Button("-", GUILayout.Width(20)))
                ...
                foreach (AnimatorControllerLayer layer in controller.layers)
                {
                    ...
                    if (m.Success)
                    {
                        ...
                        {
                            stateNameList.Add(controllerName + "/" + state.state.name);
                        }
                    }
                }
                SerializedProperty stateNamesProp = serializedObject.FindProperty("_stateNames");
                stateNamesProp.arraySize = stateNameList.Count;
                for (int i = 0; i < stateNameList.Count; i++)
                {
                    stateNamesProp.GetArrayElementAtIndex(i).stringValue = stateNameList[i];
                }
